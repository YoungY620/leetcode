...
int* findOrder(int numCourses, int** prerequisites, int prerequisitesSize, int* prerequisitesColSize, int* returnSize){
    ...
    int i;
    int *returned = (int*)malloc(sizeof(int)*numCourses);
    (*returnSize) = 0;
    ...
    while(stk_ptr != 0){
        int course = stack[--stk_ptr];
        int j;

        in_degrees[course] = -1;
        returned[(*returnSize)++] = course;

        for(j=0;j<prerequisitesSize;j++){
            if(prerequisites[j][1]==course){
                in_degrees[prerequisites[j][0]]--;
                if(in_degrees[prerequisites[j][0]] == 0){
                    in_degrees[prerequisites[j][0]]=-1;
                    stack[stk_ptr++] = prerequisites[j][0];
                }
            }
        }
    }
